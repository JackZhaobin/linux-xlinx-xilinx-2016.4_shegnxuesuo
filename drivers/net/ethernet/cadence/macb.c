...
static int macb_mii_probe(struct net_device *dev)
{
	...
	if (bp->phy_irq)                    
		phydev->irq = bp->phy_irq;      /* attach the mac to the phy */
	...
	netdev_info(dev, "attached PHY driver [%s] (mii_bus:phy_addr=%d, irq=%d)\n",
	phydev->drv->name, phydev->mdio.addr, phydev->irq);
	...
	if (mdio_np) {
		...
		if (!err && !phy_find_first(bp->mii_bus)) {
			for (i = 0; i < PHY_MAX_ADDR; i++) {
				...
				//printk("sun added: found phy addr = %d\n", i);
				if (IS_ERR(phydev)) {
					err = PTR_ERR(phydev);
					break;
				}
